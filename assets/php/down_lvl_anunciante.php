...
if (!empty($down_email) && !empty($id) && $down_senha == $senhabd['senha_usu']) {
	// se o nivel do usuario for 4 (anunciante e comum) ele volta a ser 2, se for 5(anunciante e cliente) volta a ser 3
	if ($senhabd['nivel'] == 5) {
	$att_lvl =  mysqli_query($con,"UPDATE usuario SET nivel = '3' WHERE id_usu = '$id' AND email_usu = '$down_email' AND senha_usu = '$down_senha'");
	}else{
	...
	if ($att_lvl) {
        ...
        $subject = "DEIXAR DE FAZER PROPAGANDA - QISeventos";
        $mensagem1  = "
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <div class='cont'>
            <h3>{$down_email}</h3>
            <p>Você acabou de deixar de fazer propaganda em nosso site, suas propagandas e banners não aparecerão mais em nosso site (caso seja um anunciante com seu perfil de cliente ele ainda continuará em nossas buscas e seu perfil continuará como antes de fazer propagandas), caso você não seja um cliente com seu perfil em nossas buscas você terá sua conta premium ou master excluida. Caso não seja o {$down_email} por favor entre em contato com o <a href='https://www.qiseventos.com.br/contato-sobrenos.php#us' target='_blank'>suporte</a></p>
            <p>Att: QISeventos</p>
        </div>
        <footer>
...
$subject = "[QIS] DEIXAR DE FAZER PROPAGANDA";
$mensagem1  = "Mensagem enviado pelo site da QISeventos em : SCRIPT DE DOWN_LVL_ANUNCIANTE<br/>
ERRO: {$erro}";
