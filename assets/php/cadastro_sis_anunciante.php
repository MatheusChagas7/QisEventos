...
if ($con) { 
    ...
    $query_busca = mysqli_query($con, "SELECT email_usu,senha_usu,nivel FROM usuario WHERE email_usu like '%$email%' AND senha_usu='$senha'");
    $query_busca2 = mysqli_query($con, "SELECT email_usu,senha_usu,nivel FROM usuario WHERE email_usu like '%$email%'");
    $linha = mysqli_fetch_assoc($query_busca);
    $linha2 = mysqli_fetch_assoc($query_busca2);
    // se o email for existente (igual) mude o lvl dele (4 se for 2 OU 5 se for 3, se nao for nenhum entao retorna com o email ja cadastrado), se nao crie uma conta
    if( strtolower($email) === strtolower($linha['email_usu']) ){

        if ( $linha['nivel'] == 2 ) {

            $upd_usu = mysqli_query($con, " UPDATE usuario set nivel = 4, situacao_login = 'log' WHERE email_usu = '$email' ");
            ...
            $subject = "Confirmação de cadastro para anunciantes - QISeventos";
            $mensagem1  = "
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <div class='cont'>
            <h3>Parabéns {$nome}</h3>
            <p>Você acaba de alterar a sua conta na QISeventos !
            Você manterá as suas configurações perfil, e etc. Caso tinha um perfil de cliente você ainda o terá, podendo fazer suas alterações como antes mas agora também será um anunciante e poderá fazer suas propagandas !</p>
        </div>
        <footer>
        ...
        }else if( $linha['nivel'] == 3 ) {

            $upd_usu = mysqli_query($con, " UPDATE usuario set nivel = 5, situacao_login = 'log' WHERE email_usu = '$email' ");
        ...
        <footer>
            ...
            header('Location: ../../cadastro_anunciante.php?error=email_cadastrado');
        }

    }else{
    // se chegou aqui o usuario não existia (então cria-se uma nova conta) OU existe mas a senha estava errada, então veifica, se sim então retorna informando o erro se nao prossiga e cria uma nova conta pois chegou aqui pelo primeiro motivo
    if (!empty($linha2['email_usu']) && !empty($linha2['senha_usu']) && $senha !== $linha2['senha_usu'] ) {

        header('Location: ../../cadastro_anunciante.php?error=dados_errados');

    }else{

        $query = mysqli_query($con, " INSERT INTO usuario (id_usu, nome_completo, email_usu, senha_usu, dt_nasc, sexo, nivel, pacote_usu, foto_perfil, url_persona, migracao, situacao_login) VALUES ('','$nome','$email','$senha', '$dateFormat', '$genero', '4', '0', '$foto_perfil', '$url_persona', '$migracao', 'log') ");
        ...
        header('Location: ../../index.php');
            ...
            header('Location: ../../cadastro_anunciante.php?error=foradoar');
