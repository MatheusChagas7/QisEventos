...
if ($con) {
	...
	// coloca as novas propaganda
	$query_upprop = mysqli_query($con, " UPDATE propaganda SET dt_inicio = '$dt_inicio', dt_fim = '$dt_fim' WHERE id_propaganda = '$id_prop' ");
	...
	$data['redirectURL'] = 'https://www.qiseventos.com.br/reativar_propaganda.php?done=feito';
	...
	if($xml == 'Unauthorized'){
		...
		header('Location: ../../reativar_propaganda.php?done=autenticacao');
	...
	if(count($xml -> error) > 0){
		...
		header('Location: ../../reativar_propaganda.php?done=dadosInvalidos');
		exit;
	}
	...
	$subject = "[QIS] REATIVAR_PROPAGANDA";
	$mensagem1  = "Mensagem enviado pelo site da QISeventos em : SCRIPT ENVIAR O PAGAMENTO DA PROPAGANDA PARA REATIVAR<br />
	ERRO: {$erro}";
	...
	header('Location: ../../reativar_propaganda.php?error=sim&motivo=errodatabase');
